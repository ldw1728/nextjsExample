...
//postListComponent
export default function postList({posts, pageIdx}:any){
    ...
    return (
        ...
        <Pagination totPage={totPage} pageIdx={pageIdx}/>
        </>
...
function setPostsForPage(posts:any[], pageIdx:Number){
    ...
    var ltPostIdx:number = Number(contentCnt) * Number(pageIdx); //페이지의 마지막 포스트 인덱스
    var stPostIdx:number = Number(ltPostIdx) - Number(contentCnt); //페이지의 시작 포스트 인덱스
    var pagePosts = [];
    ...
    if(posts.length > stPostIdx){ //포스트들의 갯수가 해당페이지의 시작 인덱스 보다 클때
        if(posts.length-1 <= ltPostIdx){ //포스트들의 갯수 해당페이지의 마지막 인덱스와 같거나 작을 때
            ltPostIdx = posts.length-1; //포스트들 배열의 맨 끝 인덱스를 마지막인덱스로 한다.
        }

        var pagePosts = posts.slice(stPostIdx, ltPostIdx); //자름.
    }
